...
module.exports = async (req, res) => {
  try {
    ...
    if (!shopifyStoreDomain || !shopifyAdminToken) {
      return res.status(500).json({
        error: 'Missing SHOPIFY_STORE_DOMAIN or SHOPIFY_ADMIN_TOKEN in environment variables'
      });
    }
    ...
    const products = productEdges.map(({ node: product }) => {
      ...
      return {
        ...
        productUrl: product.onlineStoreUrl || `https://${shopifyStoreDomain}`
      };
    });
